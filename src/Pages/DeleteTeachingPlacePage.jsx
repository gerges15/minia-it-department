...
import { deleteTeachingPlace, getTeachingPlaces } from "../../API/endpoints";
...
export default function DeleteTeachingPlacePage() {
    ...
    const performDelete = async (place) => {
        ...
        try {
            ...
            const detail = err.response?.data?.detail;
            setDeleteError(typeof detail === "string" ? detail : `Failed to delete "${place.name}". Please try again.`);
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto mt-8 mb-8 px-4 sm:px-0">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6">Delete Teaching Place</h1>

            <TeachingPlacesListView places={places} onActionClick={handleDeleteClick} actionType="delete" />

            {deleteSuccess && <SuccessMessage successMessage={deleteSuccess} onClose={() => setDeleteSuccess(null)} />}
            {deleteError && <ErrorMessage error={deleteError} onClose={() => setDeleteError(null)} />}
