...
import { getTeachingPlaces, updateTeachingPlace } from "../../API/endpoints";
...
export default function UpdateTeachingPlacePage() {
    ...
    const [formData, setFormData] = useState({ id: null, name: "", capacity: "", type: 0 });
    ...
    useEffect(() => {
        const fetchPlaces = async () => {
            ...
            try {
                ...
                setFetchError(
                    typeof detail === "string" ? detail : "Could not load teaching places. Please try again later."
                );
            } finally {
    ...
    const handleUpdateSubmit = async (event) => {
        ...
        try {
            ...
            setUpdateSuccess(`Teaching place "${response.data.name || formData.name}" updated successfully!`);

            setPlaces((prevPlaces) => prevPlaces.map((p) => (p.id === formData.id ? { ...p, ...response.data } : p)));
        } catch (err) {
            console.error("API Update Error:", err);
            const detail = err.response?.data?.detail;
            setUpdateError(typeof detail === "string" ? detail : "Failed to update teaching place. Please try again.");
        } finally {
            setIsUpdating(false);
        }
    };
                ...
                {formLoading ? (
                    ...
                    >
                        <h1 className="text-xl md:text-2xl font-bold text-gray-800 mb-6 text-center">
                            Update Teaching Place: <span className="text-[#7e57c2]">{formData.name}</span>
                        </h1>
                        ...
                        <div className="min-h-[2rem] mt-1">{updateError && <ErrorMessage error={updateError} />}</div>
                        <div className="min-h-[2rem] mt-1">
                            {updateSuccess && <SuccessMessage successMessage={updateSuccess} />}
                        </div>
    ...
    return (
        <div className="max-w-4xl mx-auto mt-8 mb-8 px-4 sm:px-0">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-6">Update Teaching Place</h1>
            <TeachingPlacesListView places={places} handleFunction={handleSelectPlace} />
        </div>
    );
}
