...
const ScheduleViewModal = ({ isOpen, onClose, place, schedules, isLoading }) => {
                          ...
                          {HOURS.map((hour) => (
                            <tr key={hour}>
                              ...
                              {DAYS.map((day) => {
                                ...
                                const covered = schedules.some(s => 
                                  s.day === DAYS.indexOf(day) && 
                                  s.startFrom < hour && 
                                  s.endTo > hour
                                );
                                ...
                                if (schedule) {
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="h-full w-full flex flex-col items-center justify-center bg-blue-100 border border-blue-300 rounded-lg p-2 text-xs text-blue-900 shadow-sm min-h-0" style={{height: '100%'}}>
                                        <span className="font-semibold">
                                          {formatTime(schedule.startFrom)} - {formatTime(schedule.endTo)}
                                        </span>
                                        {schedule.location && (
                                          <span className="flex items-center mt-1 text-blue-800">
                                            {schedule.location}
                                          </span>
                                        )}
                                      </div>
